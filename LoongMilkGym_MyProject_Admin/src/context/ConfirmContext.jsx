...
export function ConfirmProvider({ children }) {
  ...
  const getConfirmButtonClass = () => {
    switch (modalState.type) {
      ...
      default:
        return "flex-1 py-2.5 px-4 rounded-xl bg-[var(--color-primary)] text-black hover:bg-[var(--color-primary-hover)] text-xs font-extrabold transition-all cursor-pointer shadow-md";
    }
  };
  ...
  return (
    <ConfirmContext.Provider value={confirm}>
      ...
      {modalState.isOpen && createPortal(
        ...
        >
          <div 
            className="bg-[var(--bg-secondary)] border border-[var(--border-color)]/80 max-w-sm w-full rounded-2xl p-6 shadow-2xl flex flex-col items-center text-center animate-reactions-in"
