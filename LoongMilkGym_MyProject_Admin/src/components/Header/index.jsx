  ...
  return (
    <header className="h-16 border-b border-[var(--border-color)]/60 bg-[var(--bg-secondary)]/85 backdrop-blur-md px-6 flex items-center justify-between z-20 sticky top-0">
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-sm font-bold text-[var(--text-muted)] hidden md:inline">
          Chào mừng trở lại, <span className="text-[var(--text-primary)] font-extrabold">{userName}</span>
        </span>
      </div>

      {/* Right: Theme Switcher, Notifications, Profile Dropdown */}
      <div className="flex items-center gap-4">
        ...
        <button className="p-2 rounded-xl border border-[var(--border-color)] text-[var(--text-muted)] hover:bg-[var(--border-color)]/30 hover:text-[var(--text-color)] relative transition-all cursor-pointer">
            ...
            <>
              ...
              <div className="absolute right-0 mt-2.5 w-48 rounded-2xl bg-[var(--bg-secondary)] border border-[var(--border-color)] shadow-2xl p-2 z-40 animate-reactions-in origin-top-right">
                <div className="px-3 py-2 border-b border-[var(--border-color)]/60 mb-1">
                  ...
                  <p className="text-xs font-bold text-[var(--text-color)] truncate">{userInfo?.email || "admin@loonggym.com"}</p>
                </div>
                <button
