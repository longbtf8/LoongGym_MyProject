...
*   **Vòng đời Access Token và cơ chế xoay vòng Refresh Token**:
    - `JWT_EXPIRES_IN` hiện được cấu hình là `1h`, vì vậy access token hết hạn sau khoảng một giờ kể từ thời điểm đăng nhập hoặc làm mới token.
    - API `POST /api/auth/refresh-token` nhận refresh token thuần từ client, băm SHA-256 rồi tìm bản ghi còn hạn trong bảng `refresh_tokens`.
    - Khi refresh thành công, backend xóa bản ghi refresh token cũ và tạo một refresh token mới. Đây là cơ chế xoay vòng token nhằm giảm rủi ro nếu refresh token cũ bị lộ.
    - Cơ chế xoay vòng này yêu cầu frontend xử lý tốt tình huống nhiều request đồng thời cùng dùng refresh token cũ. Nếu không, một request thành công có thể làm các request còn lại thất bại do token cũ đã bị xóa.
    - Phần frontend đã được bổ sung lớp phục hồi: khi refresh thất bại, client kiểm tra xem tab/request khác đã ghi token mới vào `localStorage` chưa trước khi quyết định xóa phiên đăng nhập.
